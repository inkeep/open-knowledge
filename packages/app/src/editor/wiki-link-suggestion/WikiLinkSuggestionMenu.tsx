...
export function WikiLinkSuggestionMenu({
  ...
  if (loading) {
    return (
      <div
        ...
        role="status"
        aria-live="polite"
        className="w-80 max-w-[min(28rem,90vw,100%)] rounded-lg border bg-popover p-2 shadow-md text-sm text-muted-foreground"
  ...
  return (
    <div
      ...
      tabIndex={-1}
      onMouseDown={preventFocusSteal}
      className="w-80 max-w-[min(28rem,90vw,100%)] overflow-y-auto subtle-scrollbar rounded-lg border bg-popover p-1 shadow-md"
      style={{ maxHeight: 'var(--suggestion-menu-max-height, 40vh)' }}
    >
      {/*
