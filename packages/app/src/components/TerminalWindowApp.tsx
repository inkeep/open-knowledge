 ...
 * window. Mounts the SAME {@link SessionsHost} the docked terminal uses, so the
 * window is purely a placement: new-chat split button, OSC tab titles, menu
 * actions, ⌘1–9, liveness reporting, and reload rehydration are the dock's, by
 * construction. The window-shaped differences live in the host's
 * `surface: 'terminal-window'`: closing the last tab closes the window instead
 * of collapsing a panel, the tab row doubles as the macOS title bar, there is no
 * collapse control, and ⌘1–9 needs no focus-scope gate.
 *
  ...
  return (
    <>
      ...
      <SessionsHost
        ...
        // The window only exists because a PTY host does, so capability is a given.
        terminalCapable
