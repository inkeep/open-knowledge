#!/usr/bin/env bash
# Follows the shell-script conventions of precedent #22.
...
usage() {
  cat <<'EOF'
Usage: packages/app/scripts/perf-prod.sh --scenario=<name> [--runs=N] [--env="K=V ..."]

Prod-fidelity perf measurement wrapper. Orchestrates build → start CLI → run
perf:profile N times → tear down, so a developer measures against the built
artifact (equivalent to what ships to users) instead of `pnpm run dev`, whose
Vite HMR and transform overhead make the numbers uncomparable.

Required:
  --scenario=<name>   Scenario to profile. Names come from
                      packages/app/tests/perf/scenarios/; omitting the flag
                      prints the available list.

Optional:
  --runs=N            Number of profile runs (default: 5). Must be >= 1.
  --env="K=V ..."     Extra environment for each perf:profile run. Word-split, so
                      several assignments can share one flag.
  -h, --help          Show this message.

All three value flags are =-form only: `--scenario cold-pool-warm` is an unknown
argument, not a scenario.
...
What it does:
  1. Runs `pnpm run build` from the repo root (turbo-cached; a no-op when clean).
  2. Starts ONE CLI process on a kernel-assigned port —
     `node packages/cli/dist/cli.mjs start --port 0`, the project server (React
     shell + Hocuspocus + /api/* on one listener) — and polls .ok/local/server.lock
     for up to 30s for the bound port. perf:profile targets that same port.
  3. Runs `pnpm run perf:profile --scenario=<name> --target=http://localhost:<port>
     --headless` N times. Results land at
     packages/app/tests/perf/results/<scenario>.<timestamp>.json. A run that exits
     non-zero is counted and the loop continues.
  4. Sends SIGTERM, waits up to 5s for the process to exit and the lock to
     disappear, then SIGKILLs.
  5. Reads the N most recent result files. For a scenario with a registered
     primary metric, prints each value and the median; otherwise lists the files
     and the jq expression for extracting a custom metric.

Output:
  Progress lines, a SUMMARY block, and `MEDIAN: <n> ms` when a primary metric is
  registered and at least one non-negative value parsed. Partial run failures are
  reported and the median is computed from what remains.

Exit codes:
  0   the run finished; median reported when a primary metric exists
  1   usage error: unknown argument, missing --scenario, or --runs < 1
  2   --runs is not a non-negative integer, `pnpm run build` failed, or the CLI
      binary is missing after the build
  3   jq is not installed, or the server never populated .ok/local/server.lock
      with a bound port
  4   the workspace-root markers are missing, or all N perf:profile runs failed
  5   no result files found for the scenario

Not a replacement for the CI-based baseline-capture protocol — see
packages/app/tests/stress/perf-baseline-update.md §"Local prod-fidelity dry-run",
which lives in Inkeep's internal tree and is absent from the open-source
repository. Runner hardware differs; local medians are directional signal only.
EOF
}
...
if [[ -z "$METRIC" ]]; then
  ...
  # The median uses a POSIX awk insertion sort rather than GNU asort() (precedent #22(d)).
...
echo "[perf-prod]       (Inkeep-internal; absent from the open-source repository)."
