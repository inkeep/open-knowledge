/**
 * Pins the ROLLBACK_ORIGIN paired-write order: under precedent #38 the write must be ytext-first
 * and fragment-second, so a partial failure leaves ytext new and Observer B re-derives from it.
 * Reversed, a throw after the fragment write silently reverts the rollback with no error.
 */
