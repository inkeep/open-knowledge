...
describe('mechanical naming rule', () => {
  it('server-section leaves map to bare OK_<LEAF>', () => {
    ...
    expect(mechanicalEnvName(['server', 'publicUrl'])).toBe('OK_PUBLIC_URL');
...
describe('resolveEnvConfigLayer', () => {
  ...
  it('accepts http publicUrl (tailnet/LAN use) and duration idleShutdown', () => {
    const { layer } = resolveEnvConfigLayer({
      OK_PUBLIC_URL: 'http://laptop.tail:55222',
      OK_IDLE_SHUTDOWN: '45m',
    });
    expect(layer).toEqual({
      server: { publicUrl: 'http://laptop.tail:55222', idleShutdown: '45m' },
    });
  });
