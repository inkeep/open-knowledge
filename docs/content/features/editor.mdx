...
**Install detection** filters the Terminal and External apps sections to the agents you actually have; **Settings → Configure agents** decides which of those — and which In app agents — appear at all. The desktop app probes locally; the web build asks the server (`GET /api/installed-agents`), so the answer reflects the machine the server runs on — exact on `localhost`, while against a remote server (a hosted preview, SSH tunnel, remote dev) it's the server's installs that count. In app agents don't ride on that probe: they're server-hosted, so they're available wherever OpenKnowledge is. When nothing is enabled or detected, the submenu is just the **Configure agents** row.

See the [per-integration pages](../integrations/claude-code.mdx) for what each agent receives once dispatched.

The menu is organized into three sections, in the order the default follows. **In app** leads: a row for each in-app agent you've enabled, which opens the conversation in OpenKnowledge's Agents panel without leaving the app. **Terminal** (macOS and Linux desktop apps) follows: a row for every agent CLI OpenKnowledge can launch — **Claude**, **Codex**, **OpenCode**, **Cursor**, **GitHub Copilot**, **Pi**, **Antigravity**, **OpenClaw**, and **Hermes** — each running its own command in OpenKnowledge's [docked terminal](#terminal) with the same scoped prompt. A CLI the PATH probe reports absent is hidden. **External apps** comes last, listing the agent apps detected on this machine (Claude, Codex, Cursor); selecting one launches it via its deep link and hands the work out of OpenKnowledge. Because the sections can list the same name, a Terminal **Claude** (the CLI) and an External **Claude** (the app) can appear together; screen readers tell them apart by the Terminal row's "Claude CLI" accessible name. Each section renders only when it has something to launch, so on the web build and in the Windows app, which has no docked terminal, the Terminal section is absent, and a saved CLI pick degrades to the same order the default follows: an In app agent when one is available, otherwise an external app. A **Configure agents** row is always last, and is the whole menu when nothing is enabled yet.
