...
| `Cmd+J` | Show / Hide the Terminal in its current placement (macOS desktop app). With text selected, stages the passage into the AI CLI's input instead — nothing runs until you press Enter. |
| `` Ctrl+` `` | Show / Hide the Terminal too — the VS Code / Zed chord, literal `Ctrl` (macOS desktop app). Works even while a terminal holds focus. With text selected it stages the passage like `Cmd+J` instead of toggling. |
| `Cmd+Shift+J` | Open an additional terminal tab (macOS desktop app). With text selected, stages the passage into the new tab's AI CLI input. |
