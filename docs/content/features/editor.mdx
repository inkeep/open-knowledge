...
- **Link to a section.** Append `#` and the heading's slug to a doc link — `[text](./spec.md#data-model-v2)` — and the WYSIWYG editor opens the doc scrolled to that heading (the source editor ignores the fragment and opens at the top). The slug is the heading text in lowercase with accents dropped, every run of characters that aren't letters or digits replaced by one hyphen, and hyphens trimmed from both ends: `## Data model: v2` becomes `data-model-v2`, `## FAQ:` becomes `faq`, and a second identical heading in the same doc gets `-1`. Agents write the same form in chat, so a reply about one part of a long spec can take you straight to it.
...
New in-app agent sessions automatically receive the [OpenKnowledge project skill](./skills/built-in-skills.mdx#project-scope) on their first non-command message, even if you have not installed it into an editor. The added guidance stays out of your visible message text.

The **Add to prompt (+)** menu beside the field is available for every selected target. It always offers **Mention a page** and, when selected comments are waiting to be attached, **Attach comments**. With an **In app** agent selected, it also offers **Attach files**; you can add files from the menu, drop a file on the field, or paste a screenshot into it. Each attachment appears as a removable chip above the input and travels with the prompt into the new conversation. Once the prompt is sent, the chip for a doc that's still in the project opens that doc. Images can come from anywhere. Other files inside the project are sent as a path reference that the agent reads itself. A text file from outside the project, or any text file attached in a browser (which can't see file paths), travels with the message instead. Other kinds of file can't travel with the message; mention them with @ once they're in the project. Images must be PNG, JPEG, GIF, or WebP, and the images and text files that travel with one message are capped at 700 KB combined; a larger file has to be in the project, where you can mention it with @. That budget is tight for screenshots — a full-screen Retina capture is usually several megabytes and will be refused, so crop the capture or resize it first. Pick a **Terminal** CLI or an **External app** instead and the composer refuses the drop with the reason — those targets have no attachment channel — and if you attach first and switch afterwards, the send is blocked with the same explanation rather than silently dropping the file. Attachments aren't available in note windows yet, and whether an in-app agent can actually read an image is up to that agent: if it can't, OpenKnowledge shows a notification saying so once the agent's capabilities are known. The empty-state create composer on a fresh project takes attachments the same way — drop, paste, or the attach button while an **In app** agent is picked — and refuses them with the reason for **Terminal** and **External apps** targets.
...
### In-app agent release-date policies

Catalog-managed in-app agents use npm or uv to acquire their adapter. OpenKnowledge preserves your package manager's release-date policy, such as npm's `min-release-age` in your user `.npmrc` or uv's `UV_EXCLUDE_NEWER`: an older eligible release at or below the catalog version may run when the pinned release is too new. npm tries the exact stable pin first and considers older releases only after a dated refusal; uv resolves an isolated tool request bounded by the catalog version. An npm adapter resolved at the exact catalog pin is reused for as long as that OpenKnowledge server keeps running, while a fallback to an older release is re-derived an hour later; uv resolves on every launch. So a release-date policy you change while OpenKnowledge is running reaches a new conversation's fallback within the hour, and reaches an exact pin only after that server restarts: closing the editor window leaves the server running, and by default it exits on its own after 30 minutes with no editor connected.

To apply it sooner, stop that server yourself. [`ok stop`](/docs/reference/cli#manage-local-servers), run from anywhere inside the project, ends it immediately, but it declines while any editor window for that project is still connected: close those windows and run it from a terminal outside the app, or add `--force`. A terminal outside the app needs the `ok` command [on its `PATH`](/docs/reference/what-open-knowledge-writes#shell-and-path); the [built-in terminal](#terminal) carries it, but the window hosting it is itself one of those connections, so in the app the command always needs `--force`. Either way you stop the one server the whole project shares — every other window open on it drops its connection, and any in-app agent conversation running inside it stops with it — though the server does save pending changes and releases its lock on the way down.

Retrying a conversation that failed to start re-resolves that conversation's own agent right away and leaves every other conversation on the adapter it already has. Custom agent commands keep their existing behavior.

When an in-app agent needs you to sign in, the chat says so and offers what the agent supports: **Sign in** when the agent signs you in itself, and **Open terminal to sign in** (desktop app) when the agent runs its login as a terminal command or when its own CLI is installed. That terminal belongs to the chat that opened it for as long as this window stays open: when you close its tab or its shell exits, the chat retries on its own, and if the agent comes back ready, the message the sign-in held back is sent again. **Already signed in? Retry** does the same by hand, so a sign-in completed elsewhere is one press away. The terminal is never closed for you, so a failed login keeps its output on screen.

When a catalog-managed agent that OpenKnowledge launches through npx fails because npm reports that the entry in its npx cache is missing its `package.json`, OpenKnowledge deletes that one `_npx/<hash>` directory from npm's cache and relaunches the adapter once before reporting a failure. Custom agent commands and uv-managed adapters are never retried this way. Every launch that does fail, whatever its kind, is recorded with the last lines the adapter printed in [`.ok/local/acp-launch-failures.log`](../reference/what-open-knowledge-writes.mdx#while-a-project-is-open), which bug reports include.

A startup error ending with "Try again after an allowed release becomes available." means the requested adapter has no eligible release under that policy. Retry after one becomes eligible; OpenKnowledge does not disable your cooldown. Acquisition contacts your configured package registry or Python index; see [network requests](../reference/what-open-knowledge-writes.mdx#what-leaves-your-machine).

### Add your own in-app agent

In-app agents speak the [Agent Client Protocol](https://agentclientprotocol.com) (ACP): OpenKnowledge starts the agent as a subprocess, hands it the project's MCP tools and the [project skill](./skills/built-in-skills.mdx#project-scope), and gates its permission requests in the chat. The catalog lists the agents published to the ACP registry, and any other agent that speaks ACP over stdio can join it on your machine.

Create `.ok/local/acp-agents.json` inside the project, a JSON array with one entry per agent:
...
`id` may only use letters, digits, `_` and `-`; `name` is what the launcher shows; `command` plus the optional `args` are what OpenKnowledge spawns, and an optional `env` object of string values is added to the agent's environment. A bare command name is looked up on the PATH of the process that runs the OpenKnowledge server; on macOS and Linux, OpenKnowledge also asks your login shell for its PATH and merges that in, so a command that only your shell can see is still found from a desktop app launched from the Dock or Finder. That merge is skipped when the entry's `env` sets `PATH` itself, and it does not exist on Windows, so in those cases either install the command in a standard location or give its absolute path. A `command` written as a path (it contains a `/`, or a `\` on Windows) is never looked up and is used exactly as written. An entry that breaks those rules is dropped with a warning in the [server log](../reference/what-open-knowledge-writes.mdx#while-a-project-is-open), and a file that is not valid JSON, or whose top level is not an array, is ignored as a whole with the same kind of warning. The file lives under `.ok/local/`, which the project's [`.ok/.gitignore`](../reference/what-open-knowledge-writes.mdx#setting-up-a-project) keeps out of your commits, so each machine normally keeps its own list.

<Callout type="warn">
  **Review a committed `acp-agents.json` before you open the Agents panel.** For a project you did not set up, read the file's `command`, `args` and `env` first. The ignore rule only keeps your own copy out of your commits; it does not vet a file someone else committed. An entry that is switched on runs its command exactly as written, and the switch is stored by agent id rather than per project, so an id you turned on elsewhere is already on in that clone. A chat starts the agent's command as soon as its thread is created, and an Agents panel that opens empty starts a thread by itself with your default agent, or the first enabled one.
</Callout>

OpenKnowledge reads the file every time it builds the agent catalog, so once it is saved, open **Settings → Agent connections** (the **Configure agents** row in the launcher menus goes there too). A new custom agent starts folded under **In app**, because nothing has turned it on yet: expand the **Show more** button there (its label counts the folded entries) or type the agent's name in the search box, then turn it on. The agent then appears in the **In app** section of the [Open with AI](#open-with-ai) menu and the **Ask AI** agent picker, and its conversations run in the Agents panel like any other, with the same MCP tools and permission prompts. The npx cache retry described above applies only to catalog-managed agents; a custom command is launched exactly as written.

To make an agent appear for everyone without per-machine configuration, publish its manifest to the ACP registry; it then flows into the catalog on its own.
...
The terminal is also the editor's AI-chat surface: opening it starts your remembered CLI pick, or a plain shell when you have none, and the tab strip's **+** button opens another tab with whatever you picked last — a CLI chat or a plain shell. Its dropdown switches that pick and opens a tab in the thing you picked, in one action. A CLI pick sticks and is shared with the Ask AI composer; the plain-shell pick is remembered for the terminal only. In-app agents run in the Agents panel, which has its own tab strip and **+** button; picking an agent there sticks the same way and also becomes your Ask AI default. Each agent tab carries a dot for its chat's state: pulsing blue while the agent starts, signs in or works, amber while it waits for your approval or for you to sign in, green when it is ready (pulsing while it has activity you have not seen), red once the agent has stopped or hit an error, and grey for a past chat reopened from history. Hovering a tab shows its full title, that state in words, whether it has new activity, and when it was last active, and screen readers hear the state and any new activity as part of the tab's name. Picking a different agent while the open chat has not been sent yet switches that chat to the chosen agent and keeps what you have typed and attached; until that new chat has started, the agents in the **+** menu are greyed out, and the original chat stays if the new one cannot start (while a dropped file is still being read, the pick opens a new chat beside the original instead); a chat that already has a message keeps its agent, and the pick opens a new chat beside it. In an in-app chat, the **Add to prompt (+)** menu offers files and page mentions, plus selected comments when available. **Commands** appears when the selected agent provides commands and the prompt draft is empty. With text selected in the editor, `Cmd+J` on macOS, `Ctrl+J` on Windows or Linux, and `` Ctrl+` `` on any desktop platform stage the passage into the active CLI's input (or open a new CLI tab when none is running) instead of toggling — they share one handler. `Cmd+Shift+J` on macOS or `Ctrl+Shift+J` on Windows or Linux always stages it into a new CLI tab; the passage sits in the input for you to add to, and nothing is submitted until you press Enter. **Terminal ▸ New Terminal Window** (see [Terminal menu](#terminal-menu)) opens a dedicated window with the same multi-session tabs, switched with `Cmd+<number>` on macOS or `Ctrl+<number>` on Windows and Linux, and shells starting at the project root.
...
The Agents panel keeps its own open/closed memory: on the web it is remembered in your browser, on desktop with the project, where it also survives a full app restart and is restored at launch. A reload reopens the panel at that remembered state: threads already live at the reload never change it, so a panel you left closed stays closed, and a thread that goes live after the reload opens it. While the panel is closed, a dot on the **Open agents panel** tab marks live threads. A browser or a project with no recorded panel state starts with the panel closed and records the state from its first session onward.

The Agents panel's **Chat history** button opens every conversation the project has, live and closed alike, in one list. Type in its search box to filter by conversation title; the rows group into **Today** and **Older** by the local date of their last activity, newest first. Selecting a row activates that conversation's tab, and renaming or deleting a conversation works from the row's own controls, with deletion still requiring you to close an open archived chat's tab first.

Which presentation you get when opening history follows the width of the chat pane rather than the window. A wide pane docks history beside the transcript: the tab strip stays visible and usable, and the list stays open after you pick a conversation, so you close it with `Esc` or by clicking **Chat history** again. If you narrow the pane while history is docked, history closes first so the active chat remains open. Opening history in a narrow pane covers the whole chat area instead, including the tab strip, which is inert until you leave history; widening the pane while history is open moves it into the docked position without closing it. The cover list closes as soon as you pick a conversation, and a **Back** button in its header dismisses it, as does `Esc`.

To report a problem with a chat in the desktop app, choose **Report a problem with this chat…** from the chat's **Settings** menu, which stays open for this even when the agent has no settings to adjust, for example after it failed to start. Unless you uncheck **This conversation**, the report carries the newest 4 MB of the chat's messages, tool calls, and their output, plus the chat's title, the project folder's path, the document it started from, and the agent and its settings, with known secrets scrubbed. Images and other binary content in the chat are replaced by a note of their type and size, while text files attached to a message are included. The review step tells you if the chat's messages couldn't be added, in which case the report goes without the conversation. If only the title, path, document, and agent details couldn't be read, the report carries the messages without them, and the review step doesn't mention it. See [what leaves your machine](../reference/what-open-knowledge-writes.mdx#what-leaves-your-machine) for everything else a report carries.

Messages in an agent chat link the references they mention. A qualified GitHub reference such as `inkeep/open-knowledge#1284` links to that issue or pull request on github.com, or on your project's GitHub Enterprise host when the project's remote is on a host declared under [`git.hosts`](../reference/configuration.mdx), a per-machine setting. Ticket prefixes you list under [`autolinks`](../reference/configuration.mdx) in `.ok/config.yml` turn tokens such as `PRD-123` into links to the URL you give. A bare `#123` stays plain text, since agents write the same form for ordinals like "step #2", and references inside code stay as written. Hovering a link to a GitHub pull request or issue, whether written out in full or as a qualified reference, shows a card with its title, state, and author, and a pull request also shows its lines changed and whether it can merge. The card comes from GitHub's API, signed in as you when OpenKnowledge can use your GitHub sign-in, so private repositories you can read work too; with a sign-in, an open pull request also shows what it is waiting on, such as its place in the merge queue, a requested review, or failing checks. It follows the [link previews](../reference/configuration.mdx#link-previews) setting, and a reference GitHub can't answer for stays a plain link.
...
In-app agent chats in the Agents panel follow the same rule and the same setting: OpenKnowledge tools other than those five run without a prompt, the five still ask, and turning the setting off makes every OpenKnowledge tool ask again.

Shell commands in an in-app chat still prompt one at a time. When the command an agent asks to run is read-only (listing, reading, and searching files, `git status`, `log`, `diff` and other read-only git subcommands, and plain `curl` GET probes), the prompt adds **Allow read-only commands while this chat is open**. Choosing it approves that command and every later read-only command in the same chat without a prompt. A pipeline or compound command counts as read-only only when every stage does, and output can go only to `/dev/null` or into another stream (`2>&1`). Globs and variables are fine in the arguments of commands that cannot change anything, such as `cat`, `ls`, `grep` and `echo`, but `git`, `curl`, `find`, `sort` and the other commands whose options the classifier checks prompt when handed one. Command substitution, and anything else the classifier cannot prove read-only, keeps prompting. The grant shows as **Read-only allowed** on the agent settings trigger, lasts while the chat stays open (a restarted server prompts again), and **Ask again before read-only commands** in that menu revokes it.

In-app Claude Code and Codex chats can also get a browser they can drive. Turn on **Settings → Preferences → “Let agent chats use a browser”** ([`agents.browserTools`](../reference/configuration.mdx), off by default) and each Claude Code or Codex chat you start afterwards gets the [Playwright MCP](https://github.com/microsoft/playwright-mcp) server, so the agent can open pages, click, type, and take screenshots, for example to check a site it just built. Other agents' chats don't get it, because OpenKnowledge can't tell their browser actions apart from their other tools. It needs Google Chrome and Node.js installed. OpenKnowledge starts the server with the `npx` on the agent's own PATH, from a folder of its own outside the project, so npm never uses the project's `package.json`, `node_modules`, or `.npmrc`; if it can't find a usable `npx`, or the browser can't be set up, the chat starts without the browser and says so in the conversation, and the server log has the details. Without Chrome, the agent's browser actions fail with a message that Chrome is missing. The browser opens in its own window (headless on Linux with no display) with a fresh profile kept in memory, so it starts signed out of everything, and pages cannot add tools of their own. Every browser action asks for your approval, with no option to always allow it, unless the agent skips approvals itself: through its mode (the **Agent mode** item in the chat's agent settings menu) or its own permission settings, such as a Claude Code rule that allows the browser's tools. Everything the browser saves, whether or not the agent names the file, goes to a folder OpenKnowledge keeps for the chat under `~/.ok/agent-browser/`, outside the project, and is removed when you delete the chat. Pages can also carry instructions aimed at the agent, and OpenKnowledge's own read and write tools still run without asking; turn off **Let agents use OpenKnowledge without asking** ([`agents.autoApproveOkTools`](../reference/configuration.mdx)) to approve those too.
