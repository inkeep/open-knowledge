...
**Project scope** (under This project) lists the project's own `open-knowledge` skill: how to use OpenKnowledge and its MCP tools. It lives in the project folder (`.claude/skills/`, `.cursor/skills/`, and the other project-local skill dirs), so once installed it is available to whoever opens the project in that editor. Those folders are not committed by default, so a teammate cloning the repo installs it themselves — OpenKnowledge no longer creates it for them on open. When it is missing, the page says so and offers **Install**.
