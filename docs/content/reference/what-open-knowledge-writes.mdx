...
| `_npx/<hash>/` under npm's cache (`~/.npm` unless your `npm_config_cache` says otherwise) | Deleted, not written: when a catalog-managed in-app agent that OpenKnowledge launched through npx fails because npm reports that entry is missing its `package.json`, OpenKnowledge removes that one directory so npx rebuilds it on the single relaunch that follows. Never for custom agent commands or uv-managed adapters | No |
