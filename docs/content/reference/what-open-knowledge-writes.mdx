...
If you decline, only `ok` typed in an *external* terminal is affected. The MCP server and "Open with AI" launches still work without it, as does the app's built-in terminal on macOS (the built-in terminal is not yet available in the Windows and Linux apps). You can add it later from **File → Set up OpenKnowledge integrations…**. Machines that already had the block from a version before the consent dialog are treated as consented: the block is left in place and maintained.
