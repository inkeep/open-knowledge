...
| `.ok/local/stale-external-writes.json` | Private recovery snapshot containing full acknowledged, rejected, and retained document text for recently displaced writes. Protection expires after 30 minutes, but expired text is removed from disk only on a later state-persistence event, not by a timer or restart; unresolved conflicts remain until resolved | In-project, gitignored |
...
**Skills are never installed by a sweep.** OpenKnowledge writes a skill only when you ask for one — the first-launch setup dialog, the Settings toggles, Skills Studio, or `ok init`. A launch does two housekeeping passes over skills and writes nothing new: it removes superseded pre-split `open-knowledge` copies OpenKnowledge itself left in your agent folders, and it removes a bundle you switched off that is still on disk.
...
Disable both with `OK_RECLAIM_DISABLE=1`. `ok start` does not touch skill files; `ok repair-skills` is the explicit command for that.
