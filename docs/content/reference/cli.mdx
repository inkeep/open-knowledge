...
On the macOS desktop app, **App menu → Uninstall OpenKnowledge…** lets you choose projects to deinitialize, confirm removal, and answer or skip the optional "why are you leaving?" screen. The app then quits before cleanup starts, so shutdown cannot recreate its settings. A separate macOS dialog reports the result: successful cleanup guides you through dragging the app to the Trash; incomplete cleanup points to a retained log so you can repair the problem and retry.
