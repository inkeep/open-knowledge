...
| `appearance.colorTheme` | `"default" \| "dracula" \| "catppuccin-frappe" \| "catppuccin-latte" \| "monokai" \| "gruvbox" \| "solarized" \| "custom"` | (unset) | user | IDE color palette layered on top of the light/dark `theme`. `default` (the effective value when unset) follows `theme`; the named palettes are self-contained schemes that force their own light/dark mode; `custom` applies `appearance.customTheme`. Pick it in **Settings → Plugins → Themes**. |
