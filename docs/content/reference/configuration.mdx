...
| `appearance.sidebar.showSkillsSection` | boolean | `true` | project-local | Show the **Skills Studio** dock at the bottom of the file sidebar. Toggled from the sidebar's tree view options or the **Skills section** palette command. See [Skills](/docs/features/skills). |
...
| `terminal.enabled` | `boolean \| null` | `null` | project-local | Opt-out for the in-app terminal on macOS and Linux (a real OS shell at full user privilege). On by default; set `false` to disable it for this project on this machine. |
...
| `search.semantic.maxBatchSize` | positive integer | `96` | project-local | Maximum text chunks per indexing embeddings request. Lower values reduce peak memory use but require more requests. This changes transport batching only, not document chunking, vector identity, or the vector cache. Takes effect live for the next request. |
| `search.semantic.maxBatchChars` | positive integer | `96000` | project-local | Approximate character budget per indexing embeddings request. A single chunk larger than the budget is still sent alone; this setting does not split chunks or invalidate cached vectors. Takes effect live for the next request. |
| `search.semantic.docTimeoutMs` | positive integer (milliseconds) | `30000` | project-local | Timeout for each embeddings request while indexing the corpus. Search queries use a separate timeout. This transport setting takes effect live and does not invalidate cached vectors. |
...
4. (Optional) if a slow local model needs smaller requests or more time, open **Embedding request settings** and explicitly override the document batch count, character budget, or timeout. Most setups should keep the defaults (`96` / `96000` / `30000` ms). For example, one user might choose the following for a particularly slow local model — this is an override example, **not** a preset or general recommendation:
   ...
   These three controls only change HTTP request batching and the document-request timeout. They do not change chunking, query timeout, retries, vector identity, or the vector cache, so applying them live does not trigger a provider-change confirmation or a full re-embed.
