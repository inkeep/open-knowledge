...
| `remote.url` | string | (unset) | project | Superseded by `server.externalUrl` (read only while that key is absent). Public tunnel URL remote clients connect to (e.g. `https://myproject.ngrok.app`); its host is admitted through the Host-header allowlist. Used only when the server is started with the deprecated `ok start --remote` (config alone never enables remote control, and there is no server-side auth). See the [self-hosting overview](/docs/self-hosting/overview). |
| `remote.port` | number | `24550` | project | Superseded by `server.port` (read only while that key is absent). Fixed TCP port the server binds when remote control is enabled, so the tunnel's port mapping survives restarts. See the [self-hosting overview](/docs/self-hosting/overview). |
