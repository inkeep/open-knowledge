...
| `appearance.language` | `"system" \| "en" \| "zh-Hans" \| "zh-Hant" \| "hi" \| "es" \| "ar" \| "fr" \| "bn" \| "pt-BR" \| "id" \| "ur"` | (unset) | user | Interface language. `system` (the effective value when unset) follows your operating system or browser, and keeps following it. A personal preference, not project-shared — and it applies to the app's own chrome only: your documents, titles, file and folder names, frontmatter values and tags stay in the language you wrote them in. **Settings → Preferences → Language** offers the reviewed set (English, Español, 简体中文); the other locales are machine-translated and awaiting a native review, so they are set here by hand rather than picked, and `ar` / `ur` additionally wait on right-to-left layout work — the app will not select either from your OS language alone. `OK_LANG=<tag>` overrides the desktop app's native menus for a single run; the rest of the interface follows this field. Reviewing one of the machine-translated locales is what promotes it into the picker — see [Translate the interface](/docs/contribute/translations). |
| `appearance.colorThemeLight` | `"default" \| "dracula" \| "catppuccin-frappe" \| "catppuccin-latte" \| "monokai" \| "gruvbox" \| "solarized" \| "custom"` | (unset) | user | IDE color palette applied while the resolved mode is light. `default` (the effective value when unset) means no palette — the base stylesheet shows through. `custom` applies `appearance.customTheme`. Pick it in **Settings → Plugins → Themes** with the tile's sun. |
| `appearance.colorThemeDark` | `"default" \| "dracula" \| "catppuccin-frappe" \| "catppuccin-latte" \| "monokai" \| "gruvbox" \| "solarized" \| "custom"` | (unset) | user | IDE color palette applied while the resolved mode is dark — the tile's moon. `appearance.theme` chooses between the two; on `system` your OS does, so the palette follows the OS appearance. Any palette may sit in either slot, and it keeps forcing its own light/dark variant. |
| `appearance.colorTheme` | `"default" \| "dracula" \| "catppuccin-frappe" \| "catppuccin-latte" \| "monokai" \| "gruvbox" \| "solarized" \| "custom"` | (unset) | user | Legacy single-palette field, superseded by `colorThemeLight` / `colorThemeDark` above. When set, it supplies the palette for whichever of those two slots is unset, so one palette applies in both modes. Writing either slot from **Settings → Plugins → Themes** removes this key. |
...
| `remote.url` | string | (unset) | project | Public tunnel URL remote clients connect to (e.g. `https://myproject.ngrok.app`); its host is admitted through the Host-header allowlist. Used only when the server is started with `ok start --remote` (config alone never enables remote access, and there is no server-side auth). See [Remote access](/docs/deploy/remote-access). |
| `remote.port` | number | `24550` | project | Fixed TCP port the server binds when remote access is enabled, so the tunnel's port mapping survives restarts. See [Remote access](/docs/deploy/remote-access). |
