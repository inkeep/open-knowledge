...
| `editor.previewTabs` | boolean | `true` | user | Reuse one "preview" tab for single clicks in the Files tree and Skills sidebar, so browsing does not accumulate tabs. Set `false` to open every click in its own tab. A personal preference, not project-shared. Back/forward navigation and rename transitions always reuse the current tab regardless. Toggle from the Settings pane. |
