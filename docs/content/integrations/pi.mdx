...
Make sure Pi trusts the project folder. Pi does not load project-local extensions or skills from `.pi/` until the project is trusted. When you first run pi in the project, accept the trust prompt. When you use Pi inside OpenKnowledge, accepting its bridge setup prompt also grants folder trust. See [Pi folder trust](../reference/what-open-knowledge-writes.mdx#pi-folder-trust) for the trust file, `PI_CODING_AGENT_DIR`, and cleanup requirements. See Pi's [project trust documentation](https://pi.dev/docs/latest/security#project-trust) for more details.
