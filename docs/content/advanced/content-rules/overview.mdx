...
Three linters ship today. [markdownlint](/docs/advanced/content-rules/markdownlint) is the standard engine for markdown style — hard tabs, heading increments, list markers, and the rest. [Frontmatter schemas](/docs/advanced/content-rules/frontmatter) validates each doc's frontmatter against standard JSON Schema files. [OKF](/docs/plugins/okf) checks portability to the Open Knowledge Format. The system is pluggable and more linters are planned.
...
| Fix-mode keys | — | `diagnosticsArePreFix`, `reLintFailure` (only when the post-write re-lint could not report) |
