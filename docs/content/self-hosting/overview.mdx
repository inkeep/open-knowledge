...
- [**CLI**](/docs/self-hosting/methods/cli)
- [**Docker**](/docs/self-hosting/methods/docker)
...
| --- | --- |
| Desktop or CLI on your computer | [Connect the computer through a private network or tunnel](/docs/self-hosting/methods/cli#your-laptop-behind-a-tunnel). If Desktop already runs the project, skip the CLI installation step. The networking settings are the same. |
| CLI on an always-on machine | [Set up a server you manage](/docs/self-hosting/methods/cli#a-server-you-manage). |
| Docker | [Deploy the container](/docs/self-hosting/methods/docker). |
...
A private network such as Tailscale can be the access-control layer. A public URL needs an authentication edge or proxy in front of it. Follow [Authentication](/docs/self-hosting/authentication) before continuing.
...
Open the browser URL first to confirm the editor loads. Then follow [Connect remote agents](/docs/self-hosting/connecting-agents) for Claude, Cursor, Codex, and other MCP clients. Give teammates the browser URL plus any network access or credential required by your access-control setup.
