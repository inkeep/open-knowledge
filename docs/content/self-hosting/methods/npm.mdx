...
<Callout>
**On macOS and Windows**, systemd isn't available. For a simple always-on setup, run `ok start` in a terminal and configure the machine not to sleep. For unattended restarts, use the platform's service manager: launchd on macOS or Task Scheduler / a Windows service on Windows.
</Callout>
