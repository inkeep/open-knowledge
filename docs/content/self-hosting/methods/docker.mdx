...
AWS, Google Cloud, and Azure can run the image on a virtual machine with Docker. You can also use the [CLI method](/docs/self-hosting/methods/cli#a-server-you-manage) on a virtual machine. Serverless container services must provide persistent storage and support one always-on replica. Vercel and Netlify cannot host OpenKnowledge because they run request-based functions instead of one long-lived stateful process.
...
The platform domain has no login of its own, so anyone who finds the URL has full read and write access. [Add authentication](/docs/self-hosting/authentication) before opening the public domain. Once the route is protected, [try it out](/docs/self-hosting/overview#try-it-out), then [Connect remote agents](/docs/self-hosting/connecting-agents).
