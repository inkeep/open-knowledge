...
Add access control to the tunnel, private network, or proxy in front of the server. Use a tunnel or private network with the [CLI method](/docs/self-hosting/methods/cli). Use a proxy with the [Docker method](/docs/self-hosting/methods/docker). A container can also run behind a protected tunnel.
...
Run the tunnel with `--traffic-policy-file policy.yaml`. Browsers get the Google login. An agent connects by sending the Basic credential as a header. See [Connect remote agents](/docs/self-hosting/connecting-agents). Replace `CHANGE_ME` with a strong secret from `openssl rand -hex 24`. For SSO, ngrok's `--oidc` supports Okta, Azure AD, and Google Workspace.
...
Then point a custom MCP connector in Claude (iOS or Desktop) or Cursor at `https://<host>/mcp` and complete the Google login. [Connect remote agents](/docs/self-hosting/connecting-agents) has the per-client steps.
...
- **Every request returns `403`.** The request `Host` does not match `OK_EXTERNAL_URL`. Configure the proxy to preserve the client's `Host`. For Caddy, use `header_up Host {host}`. Set `OK_EXTERNAL_URL` to the public origin. See the full [proxy rules](/docs/self-hosting/methods/docker#rules-for-anything-in-front-of-the-server).
...
- **An agent gets an HTML login page back from `/mcp`.** The `/mcp` path is behind the browser login instead of a token. Route `/mcp` around the login proxy and gate it with a token, or if you use an MCP-OAuth edge, confirm the client supports it (see [Connect remote agents](/docs/self-hosting/connecting-agents)).
...
Once auth is in place, [Connect remote agents](/docs/self-hosting/connecting-agents) covers how each client presents its credential.
