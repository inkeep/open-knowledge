...
- Open the OpenKnowledge app. On first launch, approve the **Connect your AI tools to OpenKnowledge** dialog — it wires up your AI editors and makes sure the `ok` command is available in your terminal. If you skip it, re-open it later from **File → Set up OpenKnowledge integrations…**. On supported desktop installations, **Settings…** from the project navigator also opens this customization dialog; [Settings in an editor](/docs/reference/configuration) opens the full settings pane.
...
<Callout type="info">
  If your agent asks you to approve a new MCP server, approve `open-knowledge`. Cursor doesn't ask. See [Cursor → Enable the project server](/docs/integrations/cursor#enable-the-project-server).
</Callout>
