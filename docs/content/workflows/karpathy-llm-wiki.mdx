...
The structure you and the agent author (folders, titles, backlinks, a one-line purpose per folder) *is* the index. No second copy, nothing to keep in sync. The agent retrieves across it with [agentic search](../reference/agentic-search.mdx) — searching, grepping, and following backlinks over your live files, no vector database — and you teach it conventions with [skills](../features/skills/index.mdx) authored in the same editor as your docs.
...
The templates use [OKF-compatible metadata](../plugins/okf.mdx): `draft` and `stable` are the stored statuses, while provisional and canonical remain the workflow concepts.
